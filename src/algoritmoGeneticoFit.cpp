...
void GeneticAlgorithm::executarAlgoritmo(){
    ...
    for(int geracao = 0;geracao < numGeracoes;geracao++){  
        ...
        std::mt19937 gerador(seed + geracao);
        for(int i = 0; i < numFilhos; i++){
            vectorIndiviudos pais;
            pais = selecao(populacao,gerador);
            Individuo prole = crossoverOX(pais[0],pais[1],gerador);
            double mutacaoPro = distLocal(gerador);
            if(mutacaoPro < probMutacao){
                mutacao(prole,gerador);
            }
